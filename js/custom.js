...
$(document).ready(function(){
	...
	$('#playNowBtn1').click(function(e){
		var $playNowButton1 = $(this);																/* button variable */
		var $playlist = $playNowButton1.parent().parent();						/* play list section class */
		var $disk			= $playlist.children().children('.disk');			/* disk image */
		
		if ($disk.hasClass('rotating')) {
			$disk.removeClass('rotating');
			$playNowButton1.children('i').removeClass('fa-pause').addClass('fa-play');
			obj.pause();
		} else {
			$disk.addClass('rotating');
			$playNowButton1.children('i').removeClass('fa-play').addClass('fa-pause');
			obj.play();
		}
		e.preventDefault();
	});
